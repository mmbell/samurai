...
#endif
{
  ...
  */
  ...
  if (iam == 0) {
    ...
    if( !( storage = (Summarystats*) malloc( sizeof(Summarystats) * count ) ) && count )
      return GPTLerror ("%s: memory allocation failed\n", thisfunc);
...
static int merge_thread_data()
{
  ...
  for (t = 1; t < nthreads; t++) {
    ...
    if( num_newtimers ) {
      ...
      if( !( sort[0] = (char**) realloc( sort[0], (count[0] + num_newtimers) * sizeof (char *)) ) )
        return GPTLerror ("%s: memory reallocation failed\n", thisfunc);
      if( !(timerlist[0] = (char*) realloc(timerlist[0], length * (count[0] + num_newtimers) * sizeof (char)) ) )
        return GPTLerror ("%s: memory reallocation failed\n", thisfunc);
