...
bool CostFunctionXYP::writeNetCDF(const QString& netcdfFileName)
{
	...
	if (!(yVar = dataFile.add_var("y", nc3Float, latDim)))
		return NC_ERR;
	if (!(lvlVar = dataFile.add_var("pressure", nc3Float, lvlDim)))
		return NC_ERR;
	...
	Nc3Var *u, *v, *w, *wspd, *relhum, *press, *qvprime, *rhoprime, *tprime, *pprime;
	Nc3Var *vorticity, *divergence, *okuboweiss, *strain, *tpw, *rhou, *rhov, *rhow;
	Nc3Var *rho, *pheight, *temp, *qv, *h, *qr, *absVorticity;
	...
	if (!(rho = dataFile.add_var("RHOA", nc3Float, timeDim,
                                 lvlDim, latDim, lonDim)))
		return NC_ERR;
	if (!(pheight = dataFile.add_var("Z", nc3Float, timeDim,
                                   lvlDim, latDim, lonDim)))
		return NC_ERR;
	...
	if (!(dwdy = dataFile.add_var("DWDY", nc3Float, timeDim,
                                  lvlDim, latDim, lonDim)))
		return NC_ERR;
	if (!(dudz = dataFile.add_var("DUDP", nc3Float, timeDim,
                                  lvlDim, latDim, lonDim)))
		return NC_ERR;
	if (!(dvdz = dataFile.add_var("DVDP", nc3Float, timeDim,
                                  lvlDim, latDim, lonDim)))
		return NC_ERR;
	if (!(dwdz = dataFile.add_var("DWDP", nc3Float, timeDim,
                                  lvlDim, latDim, lonDim)))
		return NC_ERR;
	...
	if (!(dpdy = dataFile.add_var("DPDY", nc3Float, timeDim,
                                  lvlDim, latDim, lonDim)))
		return NC_ERR;
	if (!(dtdz = dataFile.add_var("DTDP", nc3Float, timeDim,
                                  lvlDim, latDim, lonDim)))
		return NC_ERR;
	if (!(dqdz = dataFile.add_var("DQVDP", nc3Float, timeDim,
                                  lvlDim, latDim, lonDim)))
		return NC_ERR;
	if (!(dpdz = dataFile.add_var("DZDP", nc3Float, timeDim,
                                  lvlDim, latDim, lonDim)))
		return NC_ERR;
	...
	if (!(drhody = dataFile.add_var("DRHODY", nc3Float, timeDim,
                                    lvlDim, latDim, lonDim)))
		return NC_ERR;
	if (!(drhodz = dataFile.add_var("DRHODP", nc3Float, timeDim,
                                    lvlDim, latDim, lonDim)))
		return NC_ERR;
