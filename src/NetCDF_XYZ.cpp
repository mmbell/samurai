...
double NetCDF_XYZ::calc_C(const int &i,const int &j,const int &k)
{
  ...
  double v = this->getValue(i,j,k,"v");	
  double dwdy = this->getValue(i,j,k,"dwdy");
  double w = this->getValue(i,j,k,"w");	
  double dwdz = this->getValue(i,j,k,"dwdz");
  float c_p = 1005.7;
  float g = 9.81;
...
double NetCDF_XYZ::getDerivative(const int &i,const int &j,const int &k, const std::string &var, const int &der)
{
