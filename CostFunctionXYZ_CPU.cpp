...
CostFunctionXYZ_CPU::CostFunctionXYZ_CPU(const int& numObs, const int& stateSize)
	: CostFunction(numObs, stateSize)
{
}

CostFunctionXYZ_CPU::~CostFunctionXYZ_CPU()
{
}

void CostFunctionXYZ_CPU::finalize()
{	
...
void CostFunctionXYZ_CPU::initialize(const real& imin, const real& imax, const int& idim,
...
void CostFunctionXYZ_CPU::initState()
{
...
double CostFunctionXYZ_CPU::funcValue(double* state)
{
...
void CostFunctionXYZ_CPU::funcGradient(double* state, double* gradient)
{
...
void CostFunctionXYZ_CPU::updateHCq(double* state)
{
...
void CostFunctionXYZ_CPU::updateBG()
{
...
void CostFunctionXYZ_CPU::calcInnovation()
{
...
void CostFunctionXYZ_CPU::calcHTranspose(const real* yhat, real* Astate) 
{
...
bool CostFunctionXYZ_CPU::SAtransform(real* Bstate, real* Astate)
{
	int k;
	for (int var = 0; var < varDim; var++) {
...
void CostFunctionXYZ_CPU::SBtransform(real* Ustate, real* Bstate)
{
	// Clear the Bstate
	for (int var = 0; var < varDim; var++) {
		for (int iIndex = 0; iIndex < iDim; iIndex++) {
...
void CostFunctionXYZ_CPU::SBtranspose(real* Bstate, real* Ustate)
{
...
void CostFunctionXYZ_CPU::SCtransform(real* Astate, real* Cstate)
{
...
void CostFunctionXYZ_CPU::SCtranspose(real* Cstate, real* Astate)
{
...
bool CostFunctionXYZ_CPU::setupSplines()
{
...
bool CostFunctionXYZ_CPU::outputAnalysis(const QString& suffix, real* Astate, bool updateMish)
{
...
// Basis Functions
float CostFunctionXYZ_CPU::BasisOri(int m, float x, int M, float xmin, 
...
float CostFunctionXYZ_CPU::DBasisOri(int m, float x, int M, float xmin, 
								 float DX, float DXrecip, int C)
{
	float b = 0;
	float xm = xmin + (m * DX);
...
float CostFunctionXYZ_CPU::DDBasisOri(int m, float x, int M, float xmin, 
								  float DX, float DXrecip, int C)
{
	float b = 0;
	float xm = xmin + (m * DX);
...
float CostFunctionXYZ_CPU::DDDBasisOri(int m, float x, int M, float xmin, 
								   float DX, float DXrecip, int C)
{
	float b = 0;
	float xm = xmin + (m * DX);
...
real CostFunctionXYZ_CPU::Basis(int m, real x, int M, real xmin, 
