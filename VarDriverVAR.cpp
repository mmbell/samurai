...
bool VarDriverVAR::loadBGfromFile()
{
...
bool VarDriverVAR::bilinearMish()
{
...
real VarDriverVAR::bilinearField(real xPos, real yPos, int var)
{
...
bool VarDriverVAR::setupMishAndRXform()
{
...
bool VarDriverVAR::initialize()
{
	// Run a VAR vortex background field
	cout << "Initializing VAR Background" << endl;
	...
	if (loadBG) {
		...
		int bgDim = 4*(numXnodes-1)*(numYnodes-1)*numVars;
		bgU = new real[bgDim];
	...
	costVAR = new CostFunctionVAR(obVector.size(), stateSize);
	costVAR->initialize(imin, imax, idim, jmin, jmax, jdim, ia, ja, bgU, obs); //, RnumGridpts, RXform); 
	
	return true;
}

bool VarDriverVAR::run()
{
