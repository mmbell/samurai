...
CostFunctionVAR::CostFunctionVAR(const int& numObs, const int& stateSize)
	: CostFunction(numObs, stateSize)
{
}

CostFunctionVAR::~CostFunctionVAR()
{
}

void CostFunctionVAR::finalize()
{	
...
void CostFunctionVAR::initialize(const real& imin, const real& imax, const int& idim,
...
void CostFunctionVAR::initState()
{
	...
	for (int m = 0; m < mObs; m++) {
		...
		// Not correct -- should be calculated from true k, which is not currently being passed
		real k = 0.;
...
double CostFunctionVAR::funcValue(double* state)
{
...
void CostFunctionVAR::funcGradient(double* state, double* gradient)
{
...
void CostFunctionVAR::updateHCq(double* state)
{
	...
	for (int m = 0; m < mObs; m++) {
		...
		for (int iNode = ii-1; iNode <= ii+2; ++iNode) {
			for (int jNode = jj-1; jNode <= jj+2; ++jNode) {				
				...
				ibasis = Basis(iNode, i, iDim-1, iMin, DI, DIrecip, 0, R1T2, R1T2);
				jbasis = Basis(jNode, j, jDim-1, jMin, DJ, DJrecip, 0, R1T2, R1T2);
				tempsum += stateC[varDim*iDim*jNode +varDim*iNode + 0] * ibasis * jbasis * w1;
				tempsum += stateC[varDim*iDim*jNode +varDim*iNode + 1] * ibasis * jbasis * w2;
...
void CostFunctionVAR::calcInnovation()
{
	...
	for (int m = 0; m < mObs; m++) {
		...
		for (int iNode = ii-1; iNode <= ii+2; ++iNode) {
			for (int jNode = jj-1; jNode <= jj+2; ++jNode) {				
				...
				ibasis = Basis(iNode, i, iDim-1, iMin, DI, DIrecip, 0, R1T2, R1T2);
				jbasis = Basis(jNode, j, jDim-1, jMin, DJ, DJrecip, 0, R1T2, R1T2);
				tempsum += bgState[varDim*iDim*jNode +varDim*iNode + 0] * ibasis * jbasis * w1;
				tempsum += bgState[varDim*iDim*jNode +varDim*iNode + 1] * ibasis * jbasis * w2;
...
void CostFunctionVAR::calcHTranspose(const real* yhat, real* Astate) 
{
...
#pragma omp atomic
				Astate[varDim*iDim*jIndex +varDim*iIndex + 0] 
				+= qhat * ibasis * jbasis * w1 * invError;
#pragma omp atomic
				Astate[varDim*iDim*jIndex +varDim*iIndex + 1] 
				+= qhat * ibasis * jbasis * w2 * invError;
#pragma omp atomic
...
bool CostFunctionVAR::SAtransform(real* Bstate, real* Astate)
{
	int k;
	for (int var = 0; var < varDim; var++) {
...
void CostFunctionVAR::SBtransform(real* Ustate, real* Bstate)
{
	// Clear the Bstate
	for (int var = 0; var < varDim; var++) {
		for (int iIndex = 0; iIndex < iDim; iIndex++) {
...
void CostFunctionVAR::SBtranspose(real* Bstate, real* Ustate)
{
...
void CostFunctionVAR::SCtransform(real* Astate, real* Cstate)
{
...
void CostFunctionVAR::SCtranspose(real* Cstate, real* Astate)
{
...
bool CostFunctionVAR::setupSplines()
{
...
bool CostFunctionVAR::outputAnalysis(const QString& suffix, real* Astate, bool updateMish)
{
						...
						for (int jmu = -jhalf; jmu <= jhalf; jmu++) {
							...
							for (int iNode = ii-1; iNode <= ii+2; ++iNode) {
								for (int jNode = jj-1; jNode <= jj+2; ++jNode) {				
									...
									real vcoeff = Astate[varDim*iDim*jNode +varDim*iNode];
									real ucoeff = Astate[varDim*iDim*jNode +varDim*iNode + 1];
									rhov +=  Astate[varDim*iDim*jNode +varDim*iNode + 0] * ibasis * jbasis;
									rhou +=  Astate[varDim*iDim*jNode +varDim*iNode + 1] * ibasis * jbasis;
							...
							if (updateMish and imu and jmu and ihalf and jhalf) {
								int bgJ = jIndex*2 + (jmu+1)/2;
								int bgI = iIndex*2 + (imu+1)/2;
								bgFields[varDim*(iDim-1)*2*bgJ +varDim*bgI + 0] = rhov;
								bgFields[varDim*(iDim-1)*2*bgJ +varDim*bgI + 1] = rhou;
							...
							if (!ihalf and !jhalf){
								// On the node
								fieldNodes[12*iDim*jIndex + 12*iIndex + 0] = wspd;
	...
	for (int m = 0; m < mObs; m++) {
		...
		for (int iNode = ii-1; iNode <= ii+2; ++iNode) {
			for (int jNode = jj-1; jNode <= jj+2; ++jNode) {				
				...
				ibasis = Basis(iNode, i, iDim-1, iMin, DI, DIrecip, 0, R1T2, R1T2);
				jbasis = Basis(jNode, j, jDim-1, jMin, DJ, DJrecip, 0, R1T2, R1T2);
				tempsum += Astate[varDim*iDim*jNode +varDim*iNode + 0] * ibasis * jbasis * w1;
				tempsum += Astate[varDim*iDim*jNode +varDim*iNode + 1] * ibasis * jbasis * w2;
...
// Basis Functions
float CostFunctionVAR::BasisOri(int m, float x, int M, float xmin, 
...
float CostFunctionVAR::DBasisOri(int m, float x, int M, float xmin, 
								 float DX, float DXrecip, int C)
{
	float b = 0;
	float xm = xmin + (m * DX);
...
float CostFunctionVAR::DDBasisOri(int m, float x, int M, float xmin, 
								  float DX, float DXrecip, int C)
{
	float b = 0;
	float xm = xmin + (m * DX);
...
float CostFunctionVAR::DDDBasisOri(int m, float x, int M, float xmin, 
								   float DX, float DXrecip, int C)
{
	float b = 0;
	float xm = xmin + (m * DX);
...
real CostFunctionVAR::Basis(int m, real x, int M, real xmin, 
